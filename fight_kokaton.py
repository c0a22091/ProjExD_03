...
#True は、アンチエイリアシング（anti-aliasing）と呼ばれるテキストや図形の滑らかさを向上させる処理を行うかどうかを指定するパラメータです。
#アンチエイリアシングを有効にすると、テキストや図形がより滑らかに表示され、より読みやすくなります。一方、アンチエイリアシングを無効にする（False にする）と、テキストや図形がよりシャープに表示されますが、縁取りのような不自然な効果が現れることがあります。
#したがって、True を指定することで、アンチエイリアシングを有効にし、より滑らかな表示を実現しています。
